...
class SourceLoader(object):
    ...
    def _process(self, source_path, magic_ext):
        ...
        source_orig_lines = source_file.readlines()
    ...
    def _normalize(self, sources, ext):
        '''
        ...
        if ext==common.FileExt.C or ext==common.FileExt.Java:
                ...
                for c in common.c_regex.finditer(source):
                    if c.group('noncomment'):
                        ...
                        while newlines_cnt:
                            norm_lines.append('\n')
                            newlines_cnt -= 1
            sources = ''.join(norm_lines)

        elif ext==common.FileExt.ShellScript or ext==common.FileExt.Python:
            sources = ''.join([c.group('noncomment') for c in common.shellscript_regex.finditer(sources) if c.group('noncomment')])
        elif ext==common.FileExt.Perl:
            sources = ''.join([c.group('noncomment') for c in common.perl_regex.finditer(sources) if c.group('noncomment')])
        elif ext==common.FileExt.PHP:
            norm_lines = []
            for c in common.php_regex.finditer(sources):
                if c.group('noncomment'):
                    norm_lines.append(c.group('noncomment'))
                elif c.group('multilinecomment'):
                    newlines_cnt = c.group('multilinecomment').count('\n')
            ...
            norm_lines = []
            for c in common.ruby_regex.finditer(sources):
                if c.group('noncomment'):
                    norm_lines.append(c.group('noncomment'))
                elif c.group('multilinecomment'):
                    newlines_cnt = c.group('multilinecomment').count('\n')
        ...
        # sources = common.whitespaces_regex.sub("", source)
        # 处理列表中的每个字符串元素
        sources = sources.split('\n')
        cleaned_texts = [common.whitespaces_regex.sub("", source.lower()) if isinstance(source, str) else None for source in sources]
